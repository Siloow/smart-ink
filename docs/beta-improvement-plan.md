...
| 3 — Scenes follow the user | Built 2026-09-07, needs your Supabase project | `scenes` and `render_history` tables plus two private buckets in `supabase/migrations/`; hosted scene and render stores behind the same functions the workspace calls; one-time copy of local scenes on first sign-in; Share = 30-day signed link to a render; thumbnails debounced to 1.5 s idle and shrunk to 512 px. Pose is deferred, not deleted: `poses.blend` and the arm rig exist, so wiring them is your call for a later phase. Hosted path unverified without a project. |
