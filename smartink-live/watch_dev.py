...
Lives in smartink-live/ (synced with ~/smartink-live). Run from that folder:
  cd smartink-live && blender --python watch_dev.py
...
_default_live = Path(__file__).resolve().parent
LIVE_DIR = Path(os.environ.get("SMARTINK_LIVE_DIR", _default_live)).expanduser()
